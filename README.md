...
- 新增 `买卖前 AI 终审`
  规则层先产出 `BUY / SELL / REDUCE / HOLD`，真正成交前再由实时 AI 做一轮终审，可把激进买入改成 `HOLD`，也可把过早卖出改成 `REDUCE / HOLD`。
- 新增 `持仓管理 AI 化`
  即使本轮规则层没有先产出动作，只要账户里还有可卖持仓，也会触发实时 AI 复核，判断继续持有、先减仓，还是结构走坏后卖出。
- 首页新增复核可见化
  `8600` 现在可以直接看到本轮复核了多少条、改写了多少条、原动作和最终动作分别是什么，以及 AI 给出的原因。
- 修复小仓位减仓被误拦截
  对于 `300 股 * 30%` 这类场景，系统现在会按最小 1 手执行，不再因为整手向下取整后变成 `0 股` 而误报“可卖数量不足 100 股”。
