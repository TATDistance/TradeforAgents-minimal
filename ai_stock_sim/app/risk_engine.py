...
from .models import FinalSignal, MarketQuote, RiskCheckResult
...
class RiskEngine:
    ...
    def evaluate(self, signal: FinalSignal, quote: MarketQuote, portfolio: PortfolioState) -> RiskCheckResult:
        if portfolio.drawdown >= self.settings.max_drawdown_pct:
            return RiskCheckResult(allowed=False, adjusted_qty=0, adjusted_position_pct=0.0, reject_reason="最大回撤熔断已触发", risk_state="REJECT")

        if quote.amount < self.settings.min_turnover:
            return RiskCheckResult(allowed=False, adjusted_qty=0, adjusted_position_pct=0.0, reject_reason="流动性不足", risk_state="REJECT")

        if quote.pct_change >= self.settings.limit_up_filter_pct:
            return RiskCheckResult(allowed=False, adjusted_qty=0, adjusted_position_pct=0.0, reject_reason="接近涨停，禁止追高", risk_state="REJECT")

        if quote.pct_change <= self.settings.limit_down_filter_pct and signal.action == "SELL":
            return RiskCheckResult(allowed=False, adjusted_qty=0, adjusted_position_pct=0.0, reject_reason="接近跌停，卖出成交风险过高", risk_state="REJECT")
        ...
        if signal.action == "SELL":
            ...
            if can_sell_qty <= 0:
                return RiskCheckResult(allowed=False, adjusted_qty=0, adjusted_position_pct=0.0, reject_reason="T+1 限制，可卖数量为 0", risk_state="REJECT")
            return RiskCheckResult(allowed=True, adjusted_qty=self._lot_round_down(can_sell_qty), adjusted_position_pct=1.0, risk_state="ALLOW")

        if target_value <= 0:
            return RiskCheckResult(allowed=False, adjusted_qty=0, adjusted_position_pct=0.0, reject_reason="仓位上限已用尽", risk_state="REJECT")

        raw_qty = int(target_value / max(signal.entry_price, 0.01))
        qty = self._lot_round_down(raw_qty)
        if qty <= 0:
            return RiskCheckResult(allowed=False, adjusted_qty=0, adjusted_position_pct=0.0, reject_reason="不足 100 股", risk_state="REJECT")
