...
也可以直接打开 `http://127.0.0.1:8600/` 的 `AI 实时决策首页`，先看当前阶段、执行权限、AI 动作和账户摘要；需要工程视图时，再跳到 `http://127.0.0.1:8610/` 调试面板。

`8600` 首页现在内置了一张“用户设置”卡：

- 可确认当前绑定的大模型平台
- 可确认模型名称，例如 `deepseek-chat`
- 可设置 `Base URL`
- 可设置 `API Key`
- 这套设置会和研究中心共用

研究中心不再重复显示用户设置卡，而是默认复用首页已经保存的配置。首页同时会显示“AI 来源”，帮助区分当前运行中的 AI 决策到底来自：
...
- 实时决策首页优先展示当前 AI 决策结果；研究中心里的自动选股结果仍会作为候选来源同步给实时监控
